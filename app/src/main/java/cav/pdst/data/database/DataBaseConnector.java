    ...
    public Cursor getSPTraining(int group_id){
        ...
        if (group_id == -1) {
            sql = "select sp._id,sp.sp_name,a.ci from SPORTSMAN sp " +
                    " left join (select sp_id,sum((count_training+working)-(used_training+warning_count)) as ci from ABONEMENT group by sp_id) as a on sp._id= a.sp_id order by sp.sp_name";
        }else{
            sql="select sp._id,sp.sp_name,a.ci from SPORTSMAN sp \n" +
